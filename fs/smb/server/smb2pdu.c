					...
					int s_type)
{
	...
	} else if (file_present && S_ISDIR(d_inode(path->dentry)->i_mode) &&
		   s_type == DATA_STREAM) {
				    ...
				    struct mnt_idmap *idmap)
{
	...
	posix_acl_rc = ksmbd_vfs_inherit_posix_acl(idmap, path,
						   d_inode(path->dentry->d_parent));
	if (posix_acl_rc)
	...
	if (posix_acl_rc)
		ksmbd_vfs_set_init_posix_acl(idmap, path);
	...
	if (test_share_config_flag(work->tcon->share_conf,
		...
		rc = build_sec_desc(idmap, pntsd, NULL, 0,
		...
		rc = ksmbd_vfs_set_sd_xattr(conn, idmap, path, pntsd,
					    pntsd_size, false);
		kvfree(pntsd);
		if (rc)
...
int smb2_open(struct ksmbd_work *work)
{
	...
	rc = smb2_open_get_name(work, req, posix_ctxt, &name, &stream_name,
				&s_type);
	if (rc)
		goto err_out2;

	rc = smb2_open_parse_durable(work, req, name, &lc, &dh_info,
				     &req_op_level, &fp, &stat, &file_info,
				     &reconnected);
	if (rc)
		goto err_out2;
	if (reconnected)
	...
	rc = smb2_parse_create_contexts(req, rsp, &ea_buf,
					&maximal_access_ctxt);
	if (rc)
		goto err_out2;
	...
	rc = smb2_lookup_open_path(work, req, &path, name, &file_present);
	if (rc)
		goto err_out;
	if (file_present)
		idmap = mnt_idmap(path.mnt);
	...
	rc = smb2_apply_create_contexts(work, req, fp, &query_disk_id,
					&alloc_size);
	if (rc)
		goto err_out1;
